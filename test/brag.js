...
describe("BRAG", function () {
    ...
    let deployer;
    let brag, czusd, wbnb, btcbCoin, pcsRouter, bragCzusdPair, autoRewardPool;
    before(async function () {
        ...
        btcbCoin = await ethers.getContractAt("IERC20", BTCB_TOKEN);
        ...
        console.log("deploying Brag")
        const Brag = await ethers.getContractFactory("BRAG");
        brag = await Brag.deploy(
            CZUSD_TOKEN,
            PCS_ROUTER,
            PCS_FACTORY,
        ...
        bragCzusdPair = await ethers.getContractAt("IAmmPair", bragCzusdPair_address);

        console.log("initialize autoRewardPool")
        autoRewardPool.initialize(brag.address, bragCzusdPair.address);
        ...
        await brag.approve(pcsRouter.address, ethers.constants.MaxUint256);
    ...
    it("Should deploy brag", async function () {
        const pairCzusdBal = await czusd.balanceOf(bragCzusdPair.address);
        const pairBragal = await brag.balanceOf(bragCzusdPair.address);
        const baseCzusdLocked = await brag.baseCzusdLocked();
        const totalCzusdSpent = await brag.totalCzusdSpent();
        const ownerIsExempt = await brag.isExempt(owner.address);
        const pairIsExempt = await brag.isExempt(bragCzusdPair.address);
        const tradingOpen = await brag.tradingOpen();
        expect(pairCzusdBal).to.eq(INITIAL_CZUSD_LP_WAD);
        expect(pairBragal).to.eq(INITIAL_BRAG_LP_WAD);
    ...
    it("Should burn 7% when buying and increase wad available", async function () {
        ...
        const totalCzusdSpent = await brag.totalCzusdSpent();
        const lockedCzusd = await brag.lockedCzusd();
        const availableWadToSend = await brag.availableWadToSend();
        const totalSupply = await brag.totalSupply();
        const traderBalFinal = await brag.balanceOf(trader.address);
    ...
    it("Should send reward to dev wallet", async function () {
        const devWalletBalInitial = await btcbCoin.balanceOf(manager.address);
        const autoRewardPoolBalInitial = await btcbCoin.balanceOf(autoRewardPool.address);
        const availableWadToSendInitial = await brag.availableWadToSend();
        await czusd.transfer(brag.address, parseEther("25000"));
        await brag.performUpkeep(0);
        const devWalletBalFinal = await btcbCoin.balanceOf(manager.address);
        const autoRewardPoolBalFinal = await btcbCoin.balanceOf(autoRewardPool.address);
        const availableWadToSendFinal = await brag.availableWadToSend();
        const totalCzusdSpent = await brag.totalCzusdSpent();
        const traderBal = await brag.balanceOf(trader.address);
        await brag.connect(trader).transfer(trader1.address, traderBal);
        const trader1Bal = await brag.balanceOf(trader1.address);
        await brag.connect(trader1).transfer(trader.address, trader1Bal);
        const rewardPerSecond = await autoRewardPool.rewardPerSecond();
    ...
    it("Should properly set rps on second update", async function () {
        ...
        const traderRewardBal = await btcbCoin.balanceOf(trader.address);
        const trader1RewardsReceived = await autoRewardPool.totalRewardsReceived(trader1.address);
        const trader1RewardBal = await btcbCoin.balanceOf(trader1.address);
        const autoRewardPoolBalPostRewards = await btcbCoin.balanceOf(autoRewardPool.address);
